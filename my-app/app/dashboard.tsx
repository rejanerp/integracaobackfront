...
const DashboardScreen: React.FC = () => {
  ...
  const loadFavoriteLocations = async () => {
    try {
      ...
      if (storedLocations) {
        ...
        setFavoriteLocations(parsedLocations);
      }
    } catch (error) {
  ...
  const handleMapPress = (event: MapPressEvent) => {
    if (selectingLocation) {
      ...
      router.replace(`/AddLocationScreen?latitude=${coordinate.latitude}&longitude=${coordinate.longitude}`);
  ...
  const handleFavoriteLocationPress = (index: number) => {
    if (index < 0 || index >= favoriteLocations.length) return;
      ...
      {isTablet && (
        <View style={styles.sideList}>
          ...
          {favoriteLocations.length === 0 ? (
            <Text style={styles.noFavoritesText}>Nenhum local favorito adicionado.</Text>
          ) : (
